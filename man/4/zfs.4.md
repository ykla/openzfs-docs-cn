...
此值应至少与 **zfs_vdev_async_write_active_max_dirty_percent** 相等。参见 ZFS 传输延迟。
...
旋转 vdev 上最后排队的 I/O 操作被视为具备局部性的最大距离。详见 ZFS I/O 调度器。
...
将该参数设为 0 可禁用 ZIL 对新 **xattr** = **sa** 记录的日志（前提是池启用了 **org.openzfs:zilsaxattr** 功能）。仅在 ZIL 日志或重放此记录类型存在 bug 时才需要。若功能未启用，该参数无效。
...
控制在用户命名空间中设置新 xattr 时使用的命名规则。如果 **0**（Linux 默认值），用户命名空间的 xattr 名称会带上命名空间前缀，以与 Linux 上以前版本的 ZFS 向后兼容。如果 **1**（FreeBSD 默认值），用户命名空间的 xattr 名称不带前缀，以与 illumos 和 FreeBSD 上以前版本的 ZFS 向后兼容。

无论此可调参数如何设置，本版本及未来版本的 ZFS 都可以读取任意命名规则的 xattr，但旧版本的 illumos 或 FreeBSD ZFS 无法读取以 Linux 格式写入的用户命名空间 xattr，旧版本的 Linux ZFS 也无法读取以旧 ZFS 格式写入的用户命名空间 xattr。

当覆盖 xattr 时，如果存在使用另一命名规则的同名 xattr，该 xattr 会被移除，以避免重复累积。
...
设置内核在最后一次引用释放时，是应释放 inode 结构还是将其缓存于内存中。用于测试/调试。

一个活动的 inode 结构会将多个内部 OpenZFS 结构固定在内存中，这可能导致在拥有大量不常访问文件的系统上出现大量“不可用”内存，直到内核的内存压力机制要求 OpenZFS 释放它们。

默认值 **0** 总是缓存仍存在于磁盘上的 inode。设置为 **1** 会立即释放未使用的 inode 及其关联内存，将其返回给 dbuf 缓存或 ARC 以供重用，但如果 inode 经常被驱逐并重新加载，可能会降低性能。

此参数仅在 Linux 上可用。

### **zfs_delete_dentry** = **0**|1 (int)

设置内核在不再需要 dentry 结构时，是应释放该结构还是保留在 dentry 缓存中。用于测试/调试。由于 dentry 结构持有 inode 引用，缓存的 dentry 可以将 inode 连同相关的 OpenZFS 结构无限期“固定”在内存中（参见 **zfs_delete_inode**）。

默认值 **0** 指示内核在条目及其关联 inode 不再被直接引用时进行缓存，它们会在内核的正常缓存管理过程中被回收。设置为 **1** 则指示内核在文件系统不再引用目录条目及其 inode 时立即释放它们。
