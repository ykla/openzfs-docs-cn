...
**ZFS** 是一种由 Matthew Ahrens 和 Jeff Bonwick 创建的下一代 [filesystem](https://wiki.gentoo.org/wiki/Filesystem "Filesystem")。它围绕着几个关键理念进行设计：

* 存储的管理应当是简单的。
* 冗余应当由 filesystem 处理。
* filesystem 在修复时不应被下线。
* 在代码发布之前，对最坏情况进行自动化模拟是重要的。
* 数据完整性至关重要。

ZFS 的开发始于 2001 年的 Sun Microsystems。2005 年，ZFS 作为 OpenSolaris 的一部分在 [CDDL](https://opensource.org/licenses/CDDL-1.0) 许可下发布。2007 年，Pawel Jakub Dawidek 将 ZFS 移植到了 FreeBSD。2008 年，LLNL 的 Brian Behlendorf 启动了 ZFSOnLinux 项目，将 ZFS 移植到 Linux，用于高性能计算。2010 年，Oracle 收购了 Sun Microsystems，并在当年晚些时候终止了 OpenSolaris。

Illumos 项目启动以取代 OpenSolaris，大约 2/3 的 ZFS 核心团队成员辞职，其中包括 Matthew Ahrens 和 Jeff Bonwick。许多人加入了继续开发 OpenZFS 的公司，最初是作为 Illumos 项目的一部分。Oracle 内部未辞职的 1/3 ZFS 核心团队则继续在 Oracle Solaris 中开发一个不兼容的专有 ZFS 分支。

Solaris 的第一个版本包含了一些在大规模辞职之前就已在开发中的创新改动。后续的 Solaris 版本所包含的改动则更少，且雄心不再。如今，一个不断壮大的社区在多个平台上持续开发 OpenZFS，包括 FreeBSD、Illumos、Linux 和 Mac OS X。

>**注意**
>
>Gentoo 的 ZFS 维护者不建议用户在 PORTAGE_TMPDIR 上使用 ZFS（或任何其他较为特殊的 filesystem），因为 Portage 通常会暴露缺陷。但截至 2022 年 5 月，在 ZFS 上使用应当是没有问题的。


## 特性

详细的特性列表可以在一篇 [单独的文章](https://wiki.gentoo.org/wiki/ZFS/Features "ZFS/Features") 中找到。

## 安装

### 模块

来自 [ZFSOnLinux Project](https://zfsonlinux.org/) 的树外 Linux 内核模块可用。
...
ZFS 的 Linux 内核模块与用户态工具

数据由 [Gentoo Package Database](https://packages.gentoo.org/) 提供 · 最后更新：2025-12-25 09:55
[关于 USE flags 的更多信息](https://wiki.gentoo.org/wiki/Handbook:AMD64/Working/USE)
...
**重要**
每次重新编译内核后，都需要重新编译 <span data-type="inline-memo" data-inline-memo-content="External link for the sys-fs/zfs-kmod package.">sys-fs/zfs-kmod</span>，即使内核的改动非常小也是如此。在合并内核模块之后重新编译内核时，用户可能会遇到 zpool 进入不可中断睡眠状态（无法杀死的进程）或在执行时崩溃的问题。或者，可以配合 [Distribution Kernel](https://wiki.gentoo.org/wiki/Project:Distribution_Kernel#Trying_it_out "Project:Distribution Kernel") 设置 USE=dist-kernel。

`emerge -va @module-rebuild`

### ZFS 事件守护进程通知

ZED（ZFS Event Daemon）用于监控由 ZFS 内核模块生成的事件。当一个 zevent（ZFS Event）被触发时，ZED 会运行为对应 zevent 类别启用的 ZEDLETs（ZFS Event Daemon Linkage for Executable Tasks）。
...
**注意**
对大多数配置来说，只需要前两个。zfs-load-key 用于 ZFS 加密。zfs-share 适用于使用 NFS 共享的用户，而 zfs-zed 用于 ZFS 事件守护进程，负责通过热备盘处理磁盘更换以及通过电子邮件发送故障通知。

**注意**
若要将 ZFS 用作根 filesystem，以及使用 ZFS swap，请将 zfs-import 和 zfs-mount 添加到 sysinit 级别，以便在启动或关机过程中访问 filesystem。
...
为了在启动时自动挂载 ZFS 池，需要启用以下服务和 target：
...
<span data-type="inline-memo" data-inline-memo-content="External link for the sys-fs/zfs package.">sys-fs/zfs</span> 需要内核支持 Zlib（模块或内建）。
...
你还应当在 package.mask 目录中添加一些文件，用于屏蔽特定版本的 zfs-kmod 和 zfs。这可以避免拉取其他版本的这两个包。此外，sys-fs/zfs 的版本也必须与 zfs-kmod 的版本保持一致。
...
**注意**
本节不适用于 dracut、ugrd 或 genkernel 用户。
...
下面是一个示例，演示为 6.6.2 创建 initramfs，并排除 6.1.118：
...
ZFS 已经包含了管理硬件和 filesystem 所需的所有程序，不需要任何额外工具。

### 准备

ZFS 支持使用块设备或文件。两种情况下的管理方式是相同的，但在生产环境中，ZFS 开发者建议使用块设备（最好是整块磁盘）。在试验本文所述的各种命令时，用户可以选择最方便的方式。

**重要**
为了充分利用 Advanced Format 磁盘上的块设备（即具有 4K 扇区的驱动器），强烈建议用户在创建池之前阅读 [ZFS on Linux FAQ](https://openzfs.github.io/openzfs-docs/Project%20and%20Community/FAQ.html#advanced-format-disks)。

ZFS 存储池（或 zpool）是由一个或多个虚拟设备（vdev）聚合而成的，而每个 vdev 又由一个或多个底层物理设备按特定几何结构组合而成。以下是三条非常重要的规则：

1. zpool 会跨 vdev 进行条带化，一旦其中某个 vdev 不可用，整个 zpool 也会不可用
2. 一旦 vdev 被创建，其几何结构就被固定，无法修改。重新配置 vdev 的唯一方式是销毁并重新创建
3. 要修复受损的 zpool，每个底层 vdev 都必须具备某种程度的冗余

#### 硬件考量

* ZFS 需要**直接**看到并控制支撑 zpool 的物理设备（它有自己的 I/O 调度器）。强烈不建议使用硬件 RAID，即使是多个单盘 RAID-0 虚拟磁盘，因为这种方式会削弱 ZFS 带来的优势。应始终使用 JBOD。有些控制器提供相关选项，而另一些（如较老的 LSI HBA）则需要刷入特定固件（在 LSI 的情况下称为“IT 模式固件”）才能实现 JBOD。不幸的是，有些 RAID 控制器只提供 RAID 级别而没有其他选择，这类控制器并不适合 ZFS。
* ZFS 可能具有极高的 I/O 强度：

  * 在 NAS 设备中，一个非常流行的方案是使用二手 SAS HBA 连接多块 SATA 机械硬盘（在更大规模下，推荐使用近线 SATA 硬盘）
  * 如果 zpool 使用 NVMe 模块创建，**请确保这些模块不是无 DRAM 的**（请查看数据手册）。原因有二：既可以避免明显的瓶颈，也可以避免硬件崩溃，因为某些无 DRAM 模块在 I/O 请求过载时容易卡死<sup>[[1]](https://wiki.gentoo.org/wiki/ZFS#cite_note-1)</sup>
  * 不要在 zpool 中使用所谓的 SMR（Shingled Magnetic Drives）磁盘，这类磁盘写入速度极慢，并且在 zpool resilver 过程中可能发生超时，从而阻止恢复。**在 ZFS 中始终使用 CMR 磁盘。**
...
**重要**
虽然一个 zpool 可以包含多个几何结构和容量各不相同的 vdev，但在同一个 vdev 内，所有驱动器必须是同一类型并且具有相同的特性，尤其是相同的容量和相同的扇区大小。不建议在同一个 vdev 中混合使用 SSD 和“rusty”机械硬盘，因为性能差异会带来问题。

与 RAID 类似，ZFS 提供了多种方式来创建具有（或不具有）冗余的 vdev，具体包括：

* striped（“RAID-0”）：速度最快的选项，但不提供任何磁盘损坏保护。如果一块磁盘损坏，不仅该 vdev 会失效，整个 zpool 也会失效（请记住：zpool 本质上始终是条带化的）。
* mirror（“RAID-1”）：一块磁盘是另一块的镜像。通常使用两块磁盘，但也可以使用三重（或更多）镜像，不过会牺牲空间利用效率。可容忍一块磁盘损坏（三重镜像则可容忍两块）。
...
**警告**
作为后续说明的一种替代方案，如果没有可用的物理设备，也可以使用位于现有 filesystem 之上的文件。这种方式仅适合用于实验，**绝不能在生产系统中使用**。
...
**注意**
在导出池之后，所有这些文件都会被释放，并且可以删除 /var/lib/zfs_img 目录。

如果使用 ramdisk 更为方便，上述操作的等效方式如下：

`modprobe brd rd_nr=4 rd_size=1953125`

`ls -l /dev/ram*`

这种方案需要在 Linux 内核配置中启用 `CONFIG_BLK_DEV_RAM=m`。作为最后的手段，可以使用回环设备并结合存放在 tmpfs filesystem 中的文件。第一步是创建用于存放原始镜像文件的 tmpfs：
...
每一个 ZFS 的使用场景都始于程序 /usr/sbin/zpool，它用于创建和调整 ZFS 存储池（zpool）。zpool 是一种逻辑存储空间，用于将一个或多个 vdev 组合在一起。该逻辑存储空间可以同时包含以下两种内容：

* datasets：一组文件和目录，挂载到 VFS 中，就像 xfs 或 ext4 这样的传统 filesystem 一样
* zvolumes（或 zvols）：虚拟卷，可以作为虚拟块设备访问，像机器上的任何物理硬盘一样使用，位于 /dev 下

一旦创建了 zpool，就可以对其中不同的 zvolumes 和 datasets 创建时间点快照（snapshots）。这些快照可以被浏览，甚至可以回滚，用于将内容恢复到过去的某个时间点。ZFS 的另一个重要理念是：可以通过一系列属性来管理 zpool、dataset 和 zvolume，例如加密、数据压缩、使用配额等，这些都只是属性可调内容的示例。

请记住：zpool 是在其 vdev 之间进行条带化的，因此哪怕只丢失一个 vdev，也意味着整个 zpool 的丢失。冗余发生在 vdev 层面，而不是 zpool 层面。一旦 zpool 丢失，唯一的选择就是从头重建，并从备份中恢复其内容。仅仅创建 snapshots 并不足以保证在崩溃后的恢复，这些快照还必须被发送到其他地方。后续章节将对此进行进一步说明。

**注意**
ZFS 不只是一个简单的 filesystem（按通常被接受的定义），它更像是 filesystem 与卷管理器的混合体。关于这一点的讨论超出了本文范围，只需记住这一事实，并在本文其余部分中继续使用这一常见术语即可。

#### 创建 zpool

通用语法如下：

`zpool create <poolname> [{striped,mirror,raidz,raidz2,raidz3}] /dev/blockdevice1 /dev/blockdevice2 ... [{striped,mirror,raidz,raidz2,raidz3}] /dev/blockdeviceA /dev/blockdeviceB ... [{striped,mirror,raidz,raidz2,raidz3}] /dev/blockdeviceX /dev/blockdeviceY ...`
...
一旦 zpool 创建完成，无论其架构如何，都会自动创建一个与 zpool 同名的 dataset，并直接挂载到 VFS 根目录下：
...
此时，该 dataset 只是一个空的“外壳”，尚未包含任何数据。
...
* NAME：pool 的名称
* SIZE：pool 的总大小。在 RAID-Z{1,3} vdev 的情况下，该大小已计入奇偶校验。由于 `zpool_test` 是一个镜像 vdev，这里显示的大小是单块磁盘的大小。
...
** Tip**
有关所有可用选项的详细说明，请参阅 `zfs-import(8)` 手册页。

zpool 并不会自动出现，它必须被 *导入* 后，系统才能识别并使用它。一旦 zpool 被导入，将会发生以下事情：

1. 所有 dataset（包括根 dataset）都会挂载到 VFS 中，除非 mountpoint 属性另有指定（例如 `mountpoint=none`）。如果 dataset 之间存在嵌套关系并且需要按层级方式挂载，ZFS 会自动处理这种情况。
2. zvolume 的块设备条目会在 /dev/zvol/<poolname> 下创建。
...
或者使用 `-a` 选项一次性导入所有可用的 zpool：

`zpool import -a`

如果组成某个 zpool 的所有 vdev 都可用，无论是处于干净状态还是已降级但仍可接受的状态，导入操作都应当能够成功。如果缺失了哪怕一个 vdev，该 zpool 也无法被导入，此时只能通过备份来恢复。

现在来演示前面提到过的 ALTROOT：假设需要导入 *zfs_test*，但希望其所有可挂载的 dataset 都相对于 /mnt/gentoo，而不是 VFS 根目录。这可以通过如下方式实现：
...
与导入 zpool 相反的操作称为 *导出* pool。如果没有任何对象正在使用该 pool（即 dataset 上没有打开的文件，且没有 zvolume 正在使用），则可以通过以下命令简单地导出 zpool：

`zpool export zpool_test`

此时，所有已挂载的 dataset 都会从 VFS 中卸载，与 zvolume 对应的所有块设备条目也会从 /dev 中被清除。

#### 编辑 Zpool 属性

zpool 具有若干属性（也称为 properties）。其中一部分是可以修改的，用来作为“旋钮”控制诸如数据压缩、加密等方面；而另一些属性则是内在属性，只能读取，不能修改。可以通过 `zpool` 命令的两个选项来与这些属性交互：

* `zpool get propertyname zpoolname` 用于查看指定的 propertyname（特殊的 *all* propertyname 可用于查看全部属性）
* `zpool set propertyname=propertyvalue zpoolname` 用于将 propertyname 设置为 `propertyvalue`
...
虽然不会对所有属性逐一说明，但有一点会立刻引起注意：有一些属性名称以 `feature@` 开头。这是什么？历史原因<sup>[[2]](https://wiki.gentoo.org/wiki/ZFS#cite_note-2)</sup>。

传统的 zpool 版本号编号方案在 2012 年被废弃（大致是在 Oracle 停止 OpenSolaris 并转为闭门开发 ZFS 之后），并切换为 *feature flags* 机制（zpool 的版本号同时被设置为 5000，并自此不再变化）。因此，才会在上述命令输出中看到各种 `feature@` 条目。

** Tip**
某个 ZFS 实现所支持的最新 *legacy* 版本号，可以通过 `zpool upgrade -v` 查看。对于 OpenZFS/ZFS，其支持的最新 *legacy* 版本号是 28。

在版本号编号方案仍然使用的时期，当创建一个 zpool 时，操作系统会用其所支持的最高 zpool 版本号来标记该 zpool<sup>[[3]](https://wiki.gentoo.org/wiki/ZFS#cite_note-3)</sup>。这些版本号从 1（ZFS 初始发布）开始，到 Solaris 11.4 SRU 51 所支持的 49 为止。由于 zpool 版本号在 Solaris 的各个版本之间是单调递增的，因此较新的 Solaris 版本总是可以导入由较旧 Solaris 版本创建的 zpool。但这里有一个前提：为了获得较新 Solaris 版本所带来的各种 ZFS 新特性和增强功能，必须手动升级 zpool 的版本号。一旦 zpool 版本号被升级，旧版本的 Solaris 将无法再导入该 zpool。
...
同样的逻辑也适用于 feature flags：如果某个 zpool 启用了当前系统所使用的 ZFS 版本并不支持的 feature flag，则该 zpool 将无法被导入。与（legacy）版本号机制类似，为了获得后续 OpenZFS/ZFS 版本提供的新 feature flags，必须对 zpool 进行升级（手动操作），使其支持这些新的 feature flags。一旦完成这一操作，较旧的 OpenZFS/ZFS 版本将无法再导入该 zpool。

** Important**
仅就 zpool 导入而言，feature flag 处于 `disabled` 还是 `enabled` 状态并不重要。只有在处于 `active` 状态时，才需要遵循 [Feature Flags 对照表](https://openzfs.github.io/openzfs-docs/Basic%20Concepts/Feature%20Flags.html) 中关于只读或完全不可导入的规则。

** Warning**
Feature flags 可能相当棘手：有些在被 `enabled` 后会**立刻**变为 `active`，有些一旦 `enabled` 就再也不会回到 `disabled`，还有一些一旦进入 `active` 状态就再也不会回到 `enabled`。更多细节请参阅 [zpool-features (7)](https://openzfs.github.io/openzfs-docs/man/7/zpool-features.7.html)。为了向后兼容性，建议仅在确有需要时有选择地启用这些 feature。

如果 zpool 位于 SSD 或 NVMe 设备上，启用 `autotrim` feature 可能会比较方便，这样可以让 ZFS 自动处理相关的“日常维护”工作：
...
** Important**
该变更**只会**作用于之后新写入的数据，zpool 中已有的数据保持不变。
...
磁盘被替换后，ZFS 会在后台启动一个称为 *resilvering* 的重建过程。根据 zpool 中存储的数据量不同，该过程可能需要数小时甚至数天。在此示例中数据量很小，因此 resilvering 只需几秒钟。在大型 zpool 上，resilvering 的耗时甚至可能不可接受，这正是 draid 发挥作用的场景（这是一种与 raidz 完全不同的机制，后文将介绍）。

如果不希望这是一个手动操作，那么是否可以为 zpool 配置热备盘，使其在磁盘故障时自动完成替换？这正是名为 spare 的特殊 vdev 的用途。向 zpool 添加 spare vdev 有两种方式：在创建 zpool 时指定，或在现有 zpool 上动态添加。作为热备的磁盘应当与故障磁盘保持一致。
...
如果 zpool 刚被销毁，且其所有 vdev 仍完好，可以通过 `zpool import` 并加 `-D` 选项重新导入：
...
** Warning**
技术上讲，这个操作只是将 zpool 标记为“已销毁状态”，底层 vdev 上的数据并未清除。因此，**所有未加密的数据仍可读取**。

#### 浏览 zpool 历史

有时查看 zpool 的操作历史非常有用，这时 `zpool history` 就派上用场。由于 `zpool_test` 是全新创建的，暂时没有什么有趣的记录，我们来看一个最近从远程 TrueNAS 备份恢复到工作站的生产环境 zpool 的示例：
...
1. 该 zpool 是使用 Linux live 环境（此处为 Gentoo LiveCD）创建的，当时环境设置了
   主机名为 `livecd`。
...
** Warning**
通常没有充分理由这样做。特殊 vdev 或 log vdev 可能合理，但一般会导致两者性能均下降。

** Note**
添加不匹配的 vdev 时，需要使用 `-f` 选项。
...
注意 `unsupported@com.delphix:spacemap_v2` 和 `unsupported@com.delphix:log_spacemap` 属性的值。所有不受支持的功能都以 `unsupported@` 开头。
...
* 为未来的 vdev 创建虚拟磁盘镜像（本例三个，每个 4G）：
  `<kbd>for i in {0..2}; do qemu-img create -f raw /tmp/fbsd-sparc64-ada${i}.raw 4G; done;</kbd>`
...
* 当内核启动完成后，FreeBSD 会询问控制台类型，直接按 <kbd>ENTER</kbd> 接受默认选项（VT100）

* FreeBSD 安装程序启动后，使用 <kbd>TAB</kbd> 键两次将 `LiveCD` 高亮，然后按 <kbd>ENTER</kbd> 退出安装程序并进入登录提示符

* 在登录提示符下输入 <kbd>root</kbd>（无需密码），即可进入 BASH shell 提示符

* 依次执行命令 <kbd>kldload opensolaris</kbd> **然后** <kbd>kldload zfs</kbd>（部分警告可以忽略，因为 <kbd>kldstat</kbd> 会显示已加载模块）
...
* 按 <kbd>Ctrl</kbd>+<kbd>A</kbd> <kbd>X</kbd> 终止 QEMU（无需正常关机，因为这是 live 环境），返回 Gentoo 主机 shell
* 最后，将原始镜像映射为 loop 设备：
...
ZFS 文件系统数据集可以直接通过 NFS 和/或 SMBFS/CIFS 共享。没有什么魔法，ZFS 会在后台自动调整 `/etc/exports.d/zfs.exports`（NFS）和 `/etc/samba/smb.conf`（Samba）文件的内容，使管理员无需手动修改。要共享数据集，只需设置 `sharenfs` 和/或 `sharesmb` 属性为所需协议即可。两种协议可以同时使用，但要注意：如果文件在读写模式下被打开，NFS 和 SMBFS/CIFS 的锁机制不会互相感知。
...
就像关系型数据库可以在精确时间点进行备份和恢复一样，ZFS 数据集的状态也可以被“拍照”，然后恢复，就好像自快照创建以来什么都没发生过一样。这种功能并非 ZFS 独有：BTRFS 和 NILFS2 也提供类似功能，LVM 也支持快照。Copy-on-Write 文件系统（如 ZFS、BTRFS 或 NILFS2）中的快照与 LVM 快照的主要区别在于无需预先设定大小。这不会因数据变化过多而失效，而且可以拥有几乎无限数量的快照，每个快照仅保存与前一个快照的差异。因此，如果两个快照之间没有变化，实际占用空间仅为少量元数据（几 KB 的量级）。

**重要**
与 BTRFS 快照不同，ZFS 快照是只读的。ZFS 有可写快照的概念，称为克隆（clone），稍后会详细介绍。

创建快照使用 ZFS 命令 `zfs snapshot`，后跟快照名称，遵循格式：`zpool/the/dataset/to/be/snaphotted@snapshotname`。快照名称可以是任意字母数字序列，例如 `mysnap`、`20221214` 或 `test123` 都是有效的。唯一需要遵守的规则是使用 `@` 字符作为数据集名称与快照名称的分隔符。
...
* filesystem dataset `zpool_test/testfsds1` 的大小（USED 列）为 4.50G。
* filesystem dataset `zpool_test/testfsds1` 引用的数据量（REFER 列）也是 4.50G。在此时 USED 和 REFER 两列显示相同的值，这是完全正常的。
* 快照也引用完全相同的 4.50G 数据，因此它的大小为零（快照看到的数据与 filesystem dataset 看到的完全相同，因此两者差异为零）。
...
* creation：快照创建的日期/时间
* used：几乎为零，因为快照和对应的 filesystem dataset 指向相同内容（8.24M 为快照元数据）
* referenced：前面看到的同样值 4.50G（如上解释）
* createtxg：快照创建时所在的事务组（TXG）编号（TXG 编号随时间递增）。这有助于追溯某个 filesystem dataset 的快照创建顺序，以便应对命名不一致的情况（例如 `zpool_test/testfsds1@snap1`、`zpool_test/testfsds1@mytest`、`zpool_test/testfsds1@bug412` 等）。
...
最后，`zpool_test/testfsds1@snap1` 可以通过 `zfs destroy` 命令销毁，就像 filesystem dataset 或 zvolume 一样：
...
* 在回滚 dataset 之前：
  ...
  * 确保对应的 filesystem dataset 上没有打开的文件
  * 对于 zvolume，确保它已先卸载
* 完全可以销毁中间快照，而不仅仅是最新的快照：在这种情况下，ZFS 会判断并释放所有剩余快照（以及对应的 filesystem dataset）中不再被引用的数据块。zvolumes 快照亦同理。
...
首先，重新为上一节中用于快照管理的测试 filesystem dataset 创建一个快照，因为在上一节结束时它已被销毁：
...
看到 0 字节快照在此不应感到新奇或意外。同样不应感到意外的是，快照无法被挂载（尽管可以通过伪文件系统 `/zpool_test/testfsds1/.zfs/snapshot/snap1` 访问）也无法被修改：
...
将 `clone` 作为 type 是错误的！虽然它本质上是克隆，但在行为上与普通 filesystem dataset 完全一致，ZFS 也将其视为普通 filesystem dataset。因此看到 type 为 `filesystem` 是完全正常的。

那么如何区分克隆和普通 filesystem dataset 呢？查看属性 `origin` 的设置：它指向克隆所派生的快照。作为对比，查看 `zpool_test/testfsds1` 的 origin 设置：
...
`zpool_test/testfsds1` 是独立的 filesystem dataset，而 `zpool_test/testfsds1@snap1` 与 `zpool_test/testfsds1_clone` 之间存在父子关系。由于这种关系，除非先销毁 `zpool_test/testfsds1_clone`（及其可能存在的所有快照），否则无法销毁 `zpool_test/testfsds1@snap1` 或 `zpool_test/testfsds1`。
...
由于克隆与其源 filesystem dataset 之间存在父子关系，ZFS 无法直接删除该快照。有几种方式可以解决：
...
注意变化了吗？一切被“颠倒”了，现在就像 `zpool_test/testfsds1_clone` 最初被创建，然后快照为 `zpool_test/testfsds1_clone@snap1`，再克隆出 `zpool_test/testfsds1`。重新整理后，组织结构如下：
...
在 Gentoo 环境中，快照的典型用法是实验性地测试 *"如果发生这种情况会怎样？"*：克隆生产环境的 filesystem dataset，然后挂载所需内容（就像克隆的是未压缩的 Gentoo stage 3 tarball），再 `chroot` 进入其中，提供一个可随意操作的测试环境。克隆避免了来回复制数百 GB 数据占满本地磁盘空间（ZFS 若直接复制而非克隆，将会复制所有数据块）。由于性能影响，ZFS 去重通常不被广泛使用。

作为 filesystem dataset，克隆可以使用已知命令销毁：
...
/dev/sda3 的完整名称为 `/dev/disks/by-id/wwn-0x50014ee2b3b4d492-part3`。创建名为 rpool 的 ZFS pool：
...
接下来创建一些可选的 dataset（例如用于 /home 和 /var/log）：
...
别忘了将 zfs-import 和 zfs-mount 服务添加到启动 runlevel：
