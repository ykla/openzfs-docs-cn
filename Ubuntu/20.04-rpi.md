...
* 请参见 [Ubuntu 22.04 Root on ZFS for Raspberry Pi](https://openzfs.github.io/openzfs-docs/Getting%20Started/Ubuntu/Ubuntu%2022.04%20Root%20on%20ZFS%20for%20Raspberry%20Pi.html) 以进行新安装。本指南不再接收大部分更新，仅保留供已按此指南安装的用户参考。
...
* Raspberry Pi 4 B。（如果希望在普通 PC 上安装，请参见 [Ubuntu 20.04 Root on ZFS](https://openzfs.github.io/openzfs-docs/Getting%20Started/Ubuntu/Ubuntu%2020.04%20Root%20on%20ZFS.html)）
...
* microSD 卡或 USB 磁盘。microSD 卡推荐请参考 Jeff Geerling 的 [性能比较](https://www.jeffgeerling.com/blog/2019/raspberry-pi-microsd-card-performance-comparison-2019)。使用 USB 外壳时，[确保其支持 UASP](https://github.com/geerlingguy/turing-pi-cluster/issues/11#issuecomment-647726561)。
* 一台 Ubuntu 系统（可写入 microSD 卡或 USB 磁盘），不包括目标 Raspberry Pi。

建议配备 4 GiB 内存。请勿使用去重功能，因为它需要 [大量 RAM](http://wiki.freebsd.org/ZFSTuningGuide#Deduplication)。启用去重是永久性更改，无法轻易撤销。

Raspberry Pi 3 B/B+ 可能也可运行（Pi 3 为 64 位，但 RAM 较少），但尚未经过测试。请使用下方问题链接报告你的结果（无论好坏）。

### 支持

如需帮助，请通过 [邮件列表](https://openzfs.github.io/openzfs-docs/Project%20and%20Community/Mailing%20Lists.html#mailing-lists) 或在 [Libera Chat](https://libera.chat/) 的 [#zfsonlinux](ircs://irc.libera.chat/#zfsonlinux) IRC 联系社区。如果你有与本 HOWTO 相关的错误报告或功能请求，请 [创建新 issue 并提及 @rlaager](https://github.com/openzfs/openzfs-docs/issues/new?body=@rlaager,%20I%20have%20the%20following%20issue%20with%20the%20Ubuntu%2020.04%20Root%20on%20ZFS%20for%20Raspberry%20Pi%20HOWTO:)。

### 贡献

1. Fork 并克隆： [https://github.com/openzfs/openzfs-docs](https://github.com/openzfs/openzfs-docs)
2. 安装工具：
...
4. 测试：
   ...
   ```
5. 在分支上执行 `git commit --signoff`，`git push` 并创建 pull request。提及 @rlaager。

### 加密

**警告：** 尚未在 Raspberry Pi 上进行测试过加密功能。

本指南支持三种不同的加密选项：未加密、ZFS 原生加密和 LUKS。无论选择哪种方式，所有 ZFS 功能均可完全使用。
...
Raspberry Pi 4 使用 USB 固态硬盘（SSD）比使用 microSD 卡运行速度快得多。这些指令也可用于在 USB 连接的 SSD 或其他 USB 磁盘上安装 Ubuntu。USB 磁盘有三个要求，microSD 卡不适用：

1. Raspberry Pi 的 Bootloader EEPROM 必须为 2020-09-03 或更高版本。
   要检查 bootloader 版本，请在未插入 SD 卡或未连接 USB 启动设备的情况下启动 Raspberry Pi；日期会显示在 `bootloader` 行上。（如果看不到 `bootloader` 行，说明 bootloader 太旧。）或者，在现有 Raspberry Pi 系统上运行 `sudo rpi-eeprom-update`（在 Ubuntu 上需要 `apt install rpi-eeprom`）。
   如有需要，可在现有 Raspberry Pi 系统上使用 `rpi-eeprom-update -a` 更新 bootloader 并重启。其他选项请参见 [更新 Bootloader](https://www.raspberrypi.com/documentation/computers/raspberry-pi.html#updating-the-bootloader)。
2. Raspberry Pi 必须配置为 USB 启动。Bootloader 会显示 `boot` 行；如果 `order` 中包含 `4`，说明 USB 启动已启用。
   如果尚未启用，可在现有 Raspberry Pi 系统上使用 `rpi-eeprom-config -e` 启用：设置 `BOOT_ORDER=0xf41` 并重启以应用更改。之后的启动将启用 USB 启动。
   如果没有现有系统，也可按以下步骤启用：

   * 下载 [Raspberry Pi Imager Utility](https://www.raspberrypi.com/news/raspberry-pi-imager-imaging-utility/)。
   * 将 `USB Boot` 镜像写入 microSD 卡。`USB Boot` 镜像位于 `Misc utility images` 文件夹下的 `Bootload` 中。
   * 使用 microSD 卡启动 Raspberry Pi。USB 启动应会自动启用。
3. Ubuntu 20.04 上的 U-Boot 似乎不支持 Raspberry Pi USB。[Ubuntu 20.10 可能可行](https://forums.raspberrypi.com/viewtopic.php?t=295609)。作为变通方法，Raspberry Pi bootloader 配置为直接启动 Linux。为此，Linux 内核必须未压缩。本指南会解压内核并在 `/etc/kernel/postinst.d` 添加脚本以处理内核升级。

## 第 1 步：磁盘格式化

本步骤的命令在目标 Raspberry Pi 以外的系统上运行。
...
3. 创建分区脚本：
   ...
   ```
4. 连接磁盘：
   将磁盘连接到目标 Raspberry Pi 以外的机器。如果有文件系统被自动挂载（例如 GNOME），请先卸载。确定设备名称。对于 SD 卡，设备名称几乎肯定是 `/dev/mmcblk0`；对于 USB SSD，设备名称是 `/dev/sdX`，其中 `X` 是小写字母。`lsblk` 可帮助确定设备名称。将 `DISK` 环境变量设置为设备名称：

   ```
   DISK=/dev/mmcblk0    # microSD 卡
   DISK=/dev/sdX        # USB 磁盘
   ```
   ...
   ```
   export DISKP=${DISK}p # microSD 卡
   export DISKP=${DISK}  # USB 磁盘（对于 /dev/sdX 设备，$DISKP == $DISK）
   ```

**提示**：通过 USB 读卡器连接的 microSD 卡也会显示为 `/dev/sdX`。
**警告**：以下步骤会清除磁盘上现有数据。在继续之前，请确保 `DISK` 和 `DISKP` 设置正确。
...
**提示**：如果尚未安装 ZFS 工具，可以使用 `sudo apt install zfsutils-linux` 安装。或者，也可以使用 `sudo dd if=/dev/zero of=${DISK} bs=1M status=progress` 清零整个磁盘。
...
15. 启动 Raspberry Pi。
    将 SD 卡或 USB 磁盘移至 Raspberry Pi，启动并登录（例如通过 SSH），用户名和密码均为 `ubuntu`。如果使用 SSH，请注意首次启动时 cloud-init 需要一点时间来启用密码登录。首次登录后按提示设置新密码，并使用新密码再次登录。如果你的本地 SSH 配置使用了 `ControlPersist`，可能需要先终止已有的 SSH 进程，才能进行第二次登录。
...
```
DISK=/dev/mmcblk0    # microSD 卡
DISKP=${DISK}p       # microSD 卡

DISK=/dev/sdX        # USB 磁盘
DISKP=${DISK}        # USB 磁盘
```

**警告**：设备名称在将设备移动到不同计算机或将 microSD 卡从 USB 读卡器切换到内置插槽时可能会变化。继续之前务必仔细检查设备名称。
...
**注意**：由于这是首次启动，可能会出现 `Waiting for cache lock`，这是因为 `unattended-upgrades` 正在后台运行。请耐心等待其完成。
...
* 设置 `normalization=formD` 可消除与 UTF-8 文件名规范化相关的一些边缘情况。它还隐含 `utf8only=on`，意味着仅允许 UTF-8 文件名。如果需要支持非 UTF-8 文件名，请不要使用此选项。关于仅使用 UTF-8 文件名可能带来的问题，请参见 [The problems with enforced UTF-8 only filenames](http://utcc.utoronto.ca/~cks/space/blog/linux/ForcedUTF8Filenames)。
...
3. 创建其他 dataset：
   ...
   这种 dataset 布局的主要目的是将操作系统与用户数据分离，使得回滚根文件系统不会影响用户数据。若不做额外操作，`/tmp` 将作为根文件系统的一部分存储。通过创建单独的 `/tmp` dataset，可以将 `/tmp` 数据排除在根文件系统的快照之外，也可以为 `rpool/tmp` 设置配额以限制最大空间。否则，可以使用 tmpfs（内存文件系统）。

4. 可选：忽略同步请求
   microSD 卡速度相对较慢。如果希望提升性能（尤其是在安装软件包时），可以禁用同步请求（如 `fsync()` 或 `O_[D]SYNC`）的刷新操作，代价是安全性下降。可选择以下方案：
...
6. 可选：将 `/tmp` 挂载为 tmpfs
   如果上一步已为 `/tmp` 创建了 dataset，则跳过此步骤。否则可启用 `tmp.mount` 单元，将 `/tmp` 放在 RAM 文件系统：
...
4. 可选：安装完整 GUI 环境：
   ...
   **提示**：如果安装完整 GUI 环境，通常需要如上移除 cloud-init，但改用 NetworkManager 管理网络：
...
2. 可选：仅针对 LUKS 安装，备份 LUKS 头信息：
   ...
   **提示**：如果你创建了镜像或 raidz 拓扑，请为每个 LUKS 卷（如 `luks2` 等）重复此操作。
