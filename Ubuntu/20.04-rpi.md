...
* [Ubuntu Server 20.04.4 (“Focal”) for树莓派 4](https://cdimage.ubuntu.com/releases/20.04.4/release/ubuntu-20.04.4-preinstalled-server-arm64+raspi.img.xz)
* 存储卡或 USB 磁盘。存储卡卡推荐请参考 Jeff Geerling 的 [性能比较](https://www.jeffgeerling.com/blog/2019/raspberry-pi-microsd-card-performance-comparison-2019)。当使用 USB 外置硬盘时，[确保其支持 UASP](https://github.com/geerlingguy/turing-pi-cluster/issues/11#issuecomment-647726561)。
...
2.树莓派必须配置为 USB 启动。Bootloader 会显示 `boot` 行；如果 `order` 中包含 `4`，说明 USB 启动已启用。
   ...
   * 将 `USB Boot` 镜像写入存储卡。`USB Boot` 镜像位于 `Misc utility images` 文件夹下的 `Bootload` 中。
