# 使用 ZFS 作为根文件系统的 Debian 13 (Trixie)
...
3. 创建数据集：
   ...
   >**注意：**
   >
   >由于 Debian Trixie 默认使用 tmpfs，旧版本中所有与 `/tmp` 相关的操作现已不再需要。
...
14. 可选：安装 SSH：
    ...
    ```
15. 可选：针对 ZFS 原生加密或 LUKS，配置 Dropbear 实现远程解锁：
...
基本问题是，这类控制器上的磁盘在 Linux 内核启动后才可见，而 ZoL 不会热插入池成员。详细说明见 <https://github.com/openzfs/zfs/issues/330>。
