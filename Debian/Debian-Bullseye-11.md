# 构建以 ZFS 为根文件系统的 Debian 11 (Bullseye)

## 概述

### 有更新版本

* 新安装请参阅《[根文件系统使用 ZFS 的 Debian 12 Bookworm](Debian-Bookworm-12.md)》。本指南将归档，仅作为遵循本指南完成既有安装的参考。
...
* [64 位 Debian GNU/Linux Bullseye Live CD（带 GUI，例如 gnome ISO）](https://cdimage.debian.org/mirror/cdimage/release/current-live/amd64/iso-hybrid/)
...
* `device_rebuild` 功能应该可以安全使用（raidz 上除外，因为它不兼容），但引导池本身很小，因此在实际中影响不大。
...
9. 安装 NTP 服务以同步时间。此步骤针对 Bullseye，因为在 bootstrap 过程中不会自动安装该软件包。虽然这一步对 ZFS 不是必须的，但在进行互联网访问时非常有用，因为本地时钟漂移可能导致登录失败：
